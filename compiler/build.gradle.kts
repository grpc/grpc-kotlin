...
application { mainClass.set("io.grpc.kotlin.generator.GeneratorRunner") }
...
publishing {
  publications {
    named<MavenPublication>("maven") {
      ...
      artifact(tasks.jar) { classifier = "jdk8" }
    }
  }
}
...
protobuf {
  ...
  plugins {
    ...
    id("grpckt") { path = tasks.jar.get().archiveFile.get().asFile.absolutePath }
  }
  generateProtoTasks {
    all().forEach {
